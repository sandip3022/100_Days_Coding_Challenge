...
A permutation of size N consists of all integers from 1 to N exactly once. During a swap, Blobo2 can choose any two indices i and j (1≤i,j≤N) and swap Ai? with Aj?. 
A permutation X is said to be lexicographically smaller than a permutation Y if Xi?<Yi?, where i is the first index where both the permutations differ. Input Format
The first line of input will contain a single integer T, denoting the number of test cases. The second line will contain the integer N, denoting the size of the permutation. 
The third line contains N distinct integers, the elements of the permutation A. Output Format
